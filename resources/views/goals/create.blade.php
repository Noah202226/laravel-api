    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('goals.store') }}" class="space-y-6">
            ...
            <div class="card space-y-4">
                ...
                <div class="flex gap-3">
                    <div class="w-1/4">
                        ...
                        <input type="text" name="cover_emoji" value="{{ old('cover_emoji') }}"
                               placeholder="🚀"
                            ...
                            @foreach ([
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="cover_color" value="{{ $hex }}"
                                           class="sr-only peer"
                                           {{ old('cover_color', '#6C63FF') === $hex ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}"
                           placeholder="e.g. Become a professional Laravel developer"
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              placeholder="A short summary of what this goal is about"
                              class="input w-full resize-none">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="card space-y-4">
                ...
                <p class="text-xs text-slate-500">Answering these makes you 2× more likely to follow through.</p>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">Purpose — why does this matter to you?</label>
                    <textarea name="purpose" rows="2"
                              placeholder="e.g. I want to build my own products and work remotely"
                              class="input w-full resize-none">{{ old('purpose') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">This goal is for</label>
                    <input type="text" name="goal_for" value="{{ old('goal_for') }}"
                           placeholder="e.g. My future self and my family"
                           class="input w-full">
                </div>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">Dream outcome — what does success look like?</label>
                    <textarea name="dream_outcome" rows="2"
                              placeholder="e.g. Landing my first dev job, working from home"
                              class="input w-full resize-none">{{ old('dream_outcome') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">Cost of NOT doing this</label>
                    <textarea name="fear_of_not_doing" rows="2"
                              placeholder="e.g. Staying in a job I hate, watching others build the life I want"
                              class="input w-full resize-none">{{ old('fear_of_not_doing') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">Personal mantra or quote</label>
                    <input type="text" name="motivation_quote" value="{{ old('motivation_quote') }}"
                           placeholder="e.g. Start before you are ready."
                           class="input w-full">
                </div>
            </div>
            ...
            <div class="card space-y-4">
                ...
                <p class="text-xs text-slate-500">What will you treat yourself to when this is done?</p>

                <div>
                    <label class="block text-xs text-slate-400 mb-1">Reward</label>
                    <input type="text" name="reward" value="{{ old('reward') }}"
                           placeholder="e.g. Buy a mechanical keyboard"
                           class="input w-full">
                </div>
                ...
                <div>
                    ...
                    <select name="reward_type" class="input w-full">
                        ...
                        @foreach (['experience' => '✈️ Experience', 'item' => '🛍️ Item', 'rest' => '😴 Rest', 'treat' => '🍕 Treat'] as $val => $label)
                            <option value="{{ $val }}" {{ old('reward_type') === $val ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('goals.goals') }}" class="btn bg-slate-700 hover:bg-slate-600 text-white">
                    Cancel
                </a>
            </div>
