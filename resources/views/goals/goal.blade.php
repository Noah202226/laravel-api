<x-layout>
    <div class="max-w-3xl mx-auto">
        ...
        {{-- Flash --}}
        @if (session('success'))
            <div class="mb-4 px-4 py-3 rounded-lg bg-green-500/20 text-green-400 text-sm border border-green-500/30">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="mb-2 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-slate-300 uppercase tracking-wide">Steps & Tasks</h2>
        </div>
            ...
            <div class="card mb-3">
                ...
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-slate-500 font-mono w-5">{{ $loop->iteration }}</span>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('steps.destroy', $step) }}">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-slate-600 hover:text-red-400 text-xs transition"
                                    onclick="return confirm('Delete this step and all its tasks?')">✕</button>
                        </form>
                    </div>
                </div>
                            ...
                            <li class="flex items-start gap-3 pl-1">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="text-sm {{ $task->status === 'completed' ? 'line-through text-slate-500' : '' }}">
                                            ...
                                            @if ($task->priority === 'high')
                                                <span class="text-xs text-red-400 ml-1">High</span>
                                            @endif
                                        </p>
                                        <form method="POST" action="{{ route('tasks.destroy', $task) }}">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                    class="text-slate-600 hover:text-red-400 text-xs transition flex-shrink-0">✕</button>
                                        </form>
                                    </div>
                                        ...
                                        <ul class="mt-2 space-y-1.5 pl-3 border-l border-slate-700">
                                            @foreach ($task->subtasks as $subtask)
                                                <li class="flex items-center gap-2">
                                                    ...
                                                    <span class="text-xs flex-1 {{ $subtask->is_completed ? 'line-through text-slate-500' : 'text-slate-400' }}">
                                                        {{ $subtask->title }}
                                                    </span>
                                                    <form method="POST" action="{{ route('subtasks.destroy', $subtask) }}">
                                                        @csrf @method('DELETE')
                                                        <button type="submit"
                                                                class="text-slate-700 hover:text-red-400 text-xs transition">✕</button>
                                                    </form>
                                                </li>
                                            @endforeach
                                        </ul>
                                    ...
                                    <details class="mt-2 group">
                                        <summary class="text-xs text-slate-500 hover:text-indigo-400 cursor-pointer
                                                         list-none transition select-none">
                                            + Add subtask
                                        </summary>
                                        <form method="POST" action="{{ route('subtasks.store', $task) }}"
                                            ...
                                            <input type="text" name="title" placeholder="Subtask title"
                                                   class="input flex-1 text-xs py-1.5" required>
                                            <button type="submit"
                                                    class="btn btn-primary text-xs py-1.5 px-3">Add</button>
                                        </form>
                                    </details>
                                </div>
                ...
                <details class="group mt-1">
                    <summary class="text-xs text-indigo-400 hover:text-indigo-300 cursor-pointer
                                     list-none transition select-none font-medium">
                        + Add task
                    </summary>
                    <form method="POST" action="{{ route('tasks.store', $step) }}"
                          class="mt-3 space-y-2 pl-1">
                        @csrf
                        <input type="text" name="title" placeholder="Task title"
                               class="input w-full" required>
                        <div class="flex gap-2">
                            <select name="priority" class="input flex-1 text-xs">
                                <option value="low">Low priority</option>
                                <option value="medium" selected>Medium priority</option>
                                <option value="high">High priority</option>
                            </select>
                            <input type="date" name="target_date" class="input flex-1 text-xs">
                        </div>
        ...
        @empty
            <div class="card mb-3 text-center py-8 text-slate-500 text-sm">
        ...
        <details class="group">
            <summary class="cursor-pointer list-none select-none">
                <div class="flex items-center gap-2 px-4 py-3 rounded-lg border border-dashed border-slate-700
                             hover:border-indigo-500 hover:bg-indigo-500/5 transition text-sm text-slate-400
                             hover:text-indigo-400 font-medium">
            ...
            <form method="POST" action="{{ route('steps.store', $goal) }}"
                  class="card mt-2 space-y-3">
                @csrf
                <h3 class="text-sm font-semibold text-slate-300">New Step</h3>
                <div>
                    <label class="block text-xs text-slate-400 mb-1">Step title <span class="text-red-400">*</span></label>
                    <input type="text" name="title" placeholder="e.g. PHP Foundations"
                           class="input w-full" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="target_date" class="input w-full">
                </div>
                <div class="flex gap-2">
        ...
        {{-- Meta + Actions --}}
        <div class="card mt-6 grid grid-cols-2 gap-4 text-sm mb-4">
            <div>
        ...
        <div class="flex gap-3">
            ...
            <form method="POST" action="/goals/{{ $goal->id }}">
                ...
                <button class="btn bg-red-500 hover:bg-red-600 text-white"
                        onclick="return confirm('Delete this goal and all its data?')">Delete</button>
            </form>
        </div>
